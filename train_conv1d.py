...
def load_dataset(address, step, split):
    ...
    for file in os.listdir(address):
        arr = np.array(pd.read_csv(address+file)) # [0:-1:2]
...
def evaluate_model(trainX, trainy, testX, testy):
    ...
    verbose, epochs, batch_size = 1, 20, 1000
...
def run_experiment(repeats=1):
	# load data
    trainX, trainy, testX, testy = load_dataset(address='./csv files (simple)/', step=100, split=0.8)
	# repeat experiment
    scores = list()
